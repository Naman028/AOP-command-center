...
export function createPlantRouter({ store, sessionService, auditService }) {
  ...
  router.delete("/:id", requireWrite, validateId, asyncHandler(async (req, res) => {
    if (store.useMongo) {
      ...
      const referenced = store.targets.some((target) => target.plant?.code === plant.code || target.plantId === plant.code) || store.actuals.some((actual) => actual.plant?.code === plant.code || actual.plantId === plant.code) || store.importBatches.some((batch) => batch.plantIds?.includes(plant.code));
      if (referenced) {
