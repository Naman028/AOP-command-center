...
export function ActualEntryPage() {
  const { user } = useAuth();
  const canManage = user?.permissions?.includes("ACTUALS_MANAGE");
  ...
  useEffect(() => {
    ...
    apiFetch(`/actuals?${query}`)
      .then((data) => {
        if (!ignore) {
          setActuals(data.rows);
  ...
  function selectMetric(nextMetric) {
    ...
    setEditor(null);
    setFilters((current) => ({ ...current, material: "", page: 1 }));
  }
  ...
  function newActual() {
    setEditor({
      ...
      material: "",
      actualValue: "",
      unit: isConsumption ? "EA" : "USD",
      source: "MANUAL",
      notes: ""
    });
  }

  async function saveActual(values) {
    const body = {
      plant: values.plant,
    ...
    const path = values.id ? `/actuals/${values.id}` : "/actuals";
  ...
  async function setStatus(row, isActive) {
    await apiFetch(`/actuals/${row.id}/status`, { method: "PATCH", body: JSON.stringify({ isActive }) });
    updateFilter("page", filters.page);
  }
  ...
  return (
    <main className="page">
      <div className="page-header">
        <h2>Manual Actual Entry</h2>
        {canManage && <button type="button" onClick={newActual}>Create</button>}
      </div>

      <div className="toolbar">
        {metricTypes.map(([value, label]) => (
          <button key={value} type="button" className={metricType === value ? "active-tab" : ""} onClick={() => selectMetric(value)}>{label}</button>
        ))}
      </div>
      ...
      {!state.loading && !state.error && actuals.length === 0 && <p className="state-text">No actuals found.</p>}

      {!state.loading && actuals.length > 0 && (
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {actuals.map((actual) => (
                <tr key={actual.id}>
                  <td>{actual.plant?.code}</td>
                  <td>{actual.financialYear?.label}</td>
                  <td>{actual.month}</td>
                  <td>{actual.metricType}</td>
                  <td>{actual.category}</td>
                  {isConsumption && <td>{actual.material?.code}</td>}
                  <td>{actual.actualValue}</td>
                  <td>{actual.unit}</td>
                  <td>Manual Entry</td>
                  <td><span className={actual.isActive ? "badge active" : "badge inactive"}>{actual.isActive ? "Active" : "Inactive"}</span></td>
                  {canManage && (
                    <td className="actions">
                      <button type="button" onClick={() => setEditor({ ...actual, plant: actual.plant.id, financialYear: actual.financialYear.id, material: actual.material?.id ?? "" })}>Edit</button>
                      <button type="button" onClick={() => setStatus(actual, !actual.isActive)}>{actual.isActive ? "Deactivate" : "Reactivate"}</button>
                    </td>
                  )}
                </tr>
              ))}
...
function ActualModal({ values, setValues, refs, isConsumption, onClose, onSave }) {
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <form className="modal" onSubmit={submit}>
        <h3>{values.id ? "Edit" : "Create"} Actual</h3>
        <label>
          Plant
        ...
        <label>
          Actual Value
          <input type="number" min="0.01" step="0.01" value={values.actualValue} onChange={(event) => setValues((current) => ({ ...current, actualValue: event.target.value }))} required />
        </label>
        <label>
          Unit
        ...
        <label>
          Source
          <input value="Manual Entry" readOnly />
        </label>
        <label>
          Notes
